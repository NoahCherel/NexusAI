...
export async function POST(req: NextRequest) {
    try {
        const { messages, provider, model, temperature, apiKey, systemPrompt, userPersona, enableReasoning } = await req.json();

        console.log(`[API] Request: provider=${provider}, model=${model}, reasoning=${enableReasoning}, persona=${userPersona?.name}`);
        ...
        const result = streamText({
            ...
            // We use 'providerOptions' which relies on the provider implementation.
