...
export function deriveWorldStateUpdates(
    ...
    if (itemsToAdd.length > 0 || itemsToRemove.length > 0) {
        ...
        if (itemsToRemove.length > 0) {
            update.inventory.remove = [...new Set(itemsToRemove)].filter((item) =>
                currentState.inventory.some((i) => i.toLowerCase() === item.toLowerCase())
            );
        }
    }
...
function isCharacterName(entity: string, characterName: string, userName: string): boolean {
    ...
    return (
        lower === characterName.toLowerCase() ||
...
function hasItemKeywords(text: string): boolean {
    return (
        /\b(obtain|receive|find|pick up|acquire|loot|buy|purchase|craft|take|equip|give|lose|drop|sell|destroy|broke|consumed)\b/i.test(
            text
        ) ||
        /\b(obtenir|recevoir|trouver|ramasser|acheter|fabriquer|prendre|équiper|donner|perdre|vendre|détruire|casser|consommer)\b/i.test(
            text
        )
    );
}
...
function hasLocationKeywords(text: string): boolean {
    return (
        /\b(arrive|enter|reach|travel|move to|go to|depart|leave|explore|discover|visit)\b/i.test(
            text
        ) ||
        /\b(arriver|entrer|atteindre|voyager|aller à|partir|quitter|explorer|découvrir|visiter)\b/i.test(
            text
        )
    );
}
...
function hasRelationshipKeywords(text: string): boolean {
    return (
        /\b(befriend|betray|ally|enemy|trust|love|hate|respect|fear|admire|forgive|insult|threaten|help|save)\b/i.test(
            text
        ) ||
        /\b(ami|trahir|allié|ennemi|confiance|aimer|haïr|respecter|craindre|admirer|pardonner|insulter|menacer|aider|sauver)\b/i.test(
            text
        )
    );
}
...
function matchObtainPatterns(text: string): boolean {
    return (
        /\b(obtain|receive|find|pick up|acquire|loot|buy|purchase|craft|take|equip|reward|given)\b/i.test(
            text
        ) ||
        /\b(obtenir|recevoir|trouver|ramasser|acheter|fabriquer|prendre|équiper|récompense|donné)\b/i.test(
            text
        )
    );
}
...
function matchLosePatterns(text: string): boolean {
    return (
        /\b(lose|drop|sell|destroy|broke|consumed|gave away|discard|stolen|sacrifice)\b/i.test(
            text
...
function computeRelationshipDelta(text: string, importance: number): number {
    ...
    const positivePatterns =
        /\b(befriend|ally|trust|love|respect|admire|forgive|help|save|rescue|heal|protect|grateful|thank|praise)\b/i;
    const frPositivePatterns =
        /\b(ami|allié|confiance|aimer|respecter|admirer|pardonner|aider|sauver|soigner|protéger|reconnaissant|remercier|louer)\b/i;

    // Negative relationship patterns
    const negativePatterns =
        /\b(betray|enemy|hate|fear|insult|threaten|attack|kill|murder|steal|deceive|abandon|humiliate|curse)\b/i;
    const frNegativePatterns =
        /\b(trahir|ennemi|haïr|craindre|insulter|menacer|attaquer|tuer|voler|tromper|abandonner|humilier|maudire)\b/i;
